...
variable_of_interest = 'tasmin' # add list of available name variable, tasmax, tasmin, sfcWind,rsds
unit = 'Celsius'#'mm_per_day'#'Celsius'#'mm_per_day'#'K'#'m_s-1'#'MJ.m-2.day-1'#'K'#'%' #'MJ.m-2.day-1'#sfsWind : 'm_s-1'# tas, tasmin and tasmax 'Celsius'
title_column =  u'Daily Minimum Near-Surface Air Temperature \N{DEGREE SIGN}C'#'Mean of the daily precipitation rate mm per day'#u'Daily Minimum Near-Surface Air Temperature \N{DEGREE SIGN}C'#u'Daily Near-Surface Air Temperature \N{DEGREE SIGN}C'# 'Mean of the daily precipitation rate '+unit#u'Daily Minimum Near-Surface Air Temperature \N{DEGREE SIGN}C'#'Mean of the daily precipitation rate '+unit#'Daily-Mean Near-Surface Wind Speed '+unit#'Surface Downwelling Shortwave Radiation '+unit #'Daily Near-Surface Air Temperature '+unit#u'Daily Near-Surface Air Temperature \N{DEGREE SIGN}C' #'Near-Surface Relative Humidity '+unit#'Surface Downwelling Shortwave Radiation '+unit# 'Daily-Mean Near-Surface Wind Speed '+unit#u'Daily Maximum Near-Surface Air Temperature \N{DEGREE SIGN}C'#u'Daily Near-Surface Air Temperature \N{DEGREE SIGN}C'
conversion_factor = 1#86400 #8.64 * 10**(-2) # default value is 1
# convert precipitation data from kg.m^(-2).s^(-1) to mm/day :  1 kg/m2/s = 86400 mm/day
# original units of rsds is W m-2, but need them in MJ.m-2.day-1. The conversion fatcor is 8.64 * 10**(-2)

conversion_addition = -273.15 # default value is zero
# original units of tas, tasmin and tasmax K: conversion from K to degrees C: Celsius = Kelvin - 273.15 Celsius
        ...
        for scenario in scenarios:
            if scenario == 'historical':
                if (stop_year<2015) and (start_year<2015):
                    ...
                    midx = pd.MultiIndex.from_product([(name_project[i],),(scenario,), models, (closest_value_lat[i],),(closest_value_lon[i],),time],names=['Name station','Experiment', 'Model', 'Latitude','Longitude','Date'])
...
name_project_data = np.array(['PEMBA, MZ', 'CHIMOIO, MZ', 'BEIRA, MZ']) # to compare with observation data fron the NOAA
#name_project_data=np.array(['GORONGOSA, MZ']) # to compare with precipitation observation data from SIPA and from André Görgens (Cosnultant, Water resources Management, Zutari)
name_project = pd.Series(name_project_data)

lon_projects_data = np.array([40.533, 33.467, 34.900])
#lon_projects_data = np.array([34.07824286310398])
lon_projects = pd.Series(lon_projects_data)

lat_projects_data = np.array([-12.983, -19.117, -19.800])
...
# register name of the station in the name of the file
name_stations = '' # initiate a string
for name_s in name_project:
    name_stations = name_stations + name_s[0]+name_s[1:name_s.rfind(',')].lower()

title_file = 'NEXGDDPCMIP6_at_same_emplacement_as_NOAA_station'+name_stations+'_'+variable_of_interest+'_'+str(start_year)+'-'+str(stop_year)+'.csv'
path_for_csv = os.path.join(out_path,'csv_file',variable_of_interest,variable_of_interest+'_'+unit+'_'+time_aggregations[0]+'_'+str(start_year)+'-'+str(stop_year)+'_Closest_to_NOAA')
