...
def copernicus_data(temporal_resolution,SSP,name_variable,model,year,area,path_for_file,out_path,name_area): 
    ...
    if not os.path.isdir(path_for_file):
        print('path_for_file does not exist function copernicus')
        ...
        if not os.path.isdir(file_download):
            print('file_download does not exist function copernicus')
            # function try to download from copernicus
            try_download_copernicus(temporal_resolution,SSP,name_variable,model,area,year,path_for_file)
            ...
            print('file_download does exist function copernicus')
...
def dataframe_csv_copernicus(temporal_resolution,year_str,experiments,models,out_path, global_variable, name_variable, name_project,area):    
    ...
    if not os.path.isdir(path_for_csv): # test if the data were already downloaded; if not, first part if the if is applied
        ...
        if not df.empty: # if dataframe is not empty, value were registered, the first part is run : a path to register the csv file is created, and the dataframe is registered in a csv file
            ...
            print('No value were found for the period tested function dataframe')
